...
test('accept pending applications', async t => {
  await model.pgDo(async tr => {
    ...
    const result = await model.users.acceptUserMemberships(tr, groupIdx, [
      userIdx1,
      userIdx2,
      userIdx3,
    ]);
...
test('reset resend count of expired password change token', async t => {
  await model.pgDo(async tr => {
    ...
    const originalExpires = expiryResult.rows[0].expires;
    const query =
      'UPDATE password_change_tokens SET expires = $1, resend_count = 100 WHERE token = $2';
...
test('legacy mssql password (sha512)', async t => {
  ...
  const legacyPasswordDigest = phc.serialize({
    ...
    hash: Buffer.from(
      'CF413665AC3A350E2F61EF6A8845B729CE771DD70E3FA2808C0F24CE3945A19A43F160087'
        + '60ED06D7AF5181986AC39563CE1356BA451468BD27F936FF5D1BAA9',
      'hex',
    ),
  });
  await model.pgDo(async tr => {
