...
export default class Permissions {
  ...
  public async delete(tr: Transaction, permissionIdx: number): Promise<number> {
    ...
    const result = await tr.query<{ idx: number }>(query, [permissionIdx]);
  ...
  public async addPermissionRequirement(
    tr: Transaction,
    groupIdx: number,
    ...
    const result = await tr.query<{ idx: number }>(query, [groupIdx, permissionIdx]);
    return result.rows[0].idx;
  }
  ...
  public async deletePermissionRequirement(tr: Transaction, idx: number): Promise<number> {
    ...
    const result = await tr.query<{ idx: number }>(query, [idx]);
    if (result.rows.length === 0) {
      throw new NoSuchEntryError();
    }
    return result.rows[0].idx;
  ...
  public async checkUserHavePermission(
    tr: Transaction,
    userIdx: number,
    permissionIdx: number,
  ): Promise<boolean> {
    ...
    const userReachableGroups = Array.from(
      await this.model.users.getUserReachableGroups(tr, userIdx),
    );
