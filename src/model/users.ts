...
export default class Users {
  ...
  public async getUserIdxByEmailAddress(
    tr: Transaction,
    emailLocal: string,
    emailDomain: string,
  ): Promise<number> {
  ...
  public async generatePasswordChangeToken(tr: Transaction, userIdx: number): Promise<string> {
    ...
    const query =
      'INSERT INTO password_change_tokens AS p(user_idx, token, expires) VALUES ($1, $2, $3) '
  ...
  public async resetResendCountIfExpired(tr: Transaction, userIdx: number): Promise<void> {
    const query =
      'UPDATE password_change_tokens SET resend_count = 0 WHERE user_idx = $1 AND expires <= now()';
  ...
  public async addUserMembership(
    tr: Transaction,
    userIdx: number,
    groupIdx: number,
  ): Promise<number> {
  ...
  public async hasUserMembership(
    tr: Transaction,
    userIdx: number,
    groupIdx: number,
  ): Promise<boolean> {
  ...
  public async getAllUserMemberships(
    tr: Transaction,
    userIdx: number,
  ...
  public async getAllMembershipUsers(
    tr: Transaction,
    groupIdx: number,
  ...
  public async addPendingUserMembership(
    tr: Transaction,
    userIdx: number,
    groupIdx: number,
  ): Promise<number> {
  ...
  public async deletePendingUserMembership(
    tr: Transaction,
  ...
  public async hasPendingUserMembership(
    tr: Transaction,
    userIdx: number,
    groupIdx: number,
  ): Promise<boolean> {
  ...
  public async getPendingUserMembership(
    tr: Transaction,
    userIdx: number,
    groupIdx: number,
  ): Promise<number> {
  ...
  public async acceptUserMemberships(
    tr: Transaction,
    groupIdx: number,
    userIdx: Array<number>,
  ): Promise<number> {
  ...
  public async rejectUserMemberships(
    tr: Transaction,
    groupIdx: number,
    userIdx: Array<number>,
  ): Promise<number> {
    const query1 =
      'DELETE FROM pending_user_memberships WHERE group_idx = $1 AND user_idx = ANY($2) RETURNING user_idx';
    const query2 =
      'DELETE FROM user_memberships WHERE group_idx = $1 AND user_idx = ANY($2) RETURNING user_idx';
  ...
  public async getUserReachableGroups(tr: Transaction, userIdx: number): Promise<Set<number>> {
    ...
    for (const userMembership of userMemberships) {
      const reachableGroups = await this.model.groups.getGroupReachableArray(
        tr,
        userMembership.groupIdx,
      );
  ...
  public async getStudentNumbersByUserIdx(
    tr: Transaction,
    userIdx: number,
  ...
  private generatePosixCache(users: Array<User>): void {
    let passwd = '';
    let group =
      `${this.model.config.posix.userGroupName}:x:${this.model.config.posix.userGroupGid}:`;

    const usernames: Array<string> = [];
    users.forEach(user => {
      if (user.username) {
        passwd += `${user.username}:x:${user.uid.toString()}:`
