...
model.pgDo(async c => {
  const emailAddressIdx = await model.emailAddresses.create(c, 'new', 'bacchus.snucse.org')
  return await model.users.create(c, 'bacchus', 'password_digest', 'bacchus', emailAddressIdx)
